...
- [x] 中文：联动验收完成。
- [x] English: Integration acceptance is completed.
...
- [x] 该 issue 涉及跨产品联动与内容效果，需要 John 最终确认 `passed`
- [x] This issue affects cross-product integration and content quality, so John must give final `passed`
...
- 最终验收结论：John 已明确给出 `passed`，`JOH-193` 可进入 `Done`
